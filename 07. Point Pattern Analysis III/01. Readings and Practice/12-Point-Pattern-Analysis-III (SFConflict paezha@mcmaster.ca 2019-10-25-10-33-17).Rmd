...
The function `owin` is used to specify this window for observation of the point pattern. 
...
For clarification on how to create a 'ppp' object see reading 08, point pattern analysis I. 
...
The 'split' function is used to divide data in the vector into groups. 
...
knitr::include_graphics(rep("Point Pattern Analysis-III-Figure-1.png"))
```
...
Another useful tool is a _Stienen diagram_. A Steinen diagram is essentially a proportional symbol plot of the events. The size of symbols are proportional to the distance to their nearest neighbor. For example, for "Pattern 1" in `pp0.ppp` (Notice the use of %mark% to add an attribute to the `ppp` object; the attribute is the distance to the nearest neighbor):
```{r}
...
When the empirical $\hat{G}(x)$ is greater than the theoretical function, this suggests that the events tend to be _closer_ than expected, compared to the null landscape (clustered pattern). On the contrary, when the empirical function is less than the theoretical function, this would suggest that the events tend to be _further_ than expected, compared to the null landscape (dispersed pattern).
