...
```{r}
#This function simulates the outcome of flipping a coin. Here, we are simulating the result for flipping heads, which has a probability of 0.5. 
...
```{r}
#This function generates random numbers based on the given arguments. 
rnorm(1, mean = 0, sd = 1)
```
...
```{r}
#expand.grid created a landscape from all combinations of factor variables. Here, our landscape ranges from 0 to 1, increasing by 0.05
...
```{r}
#nrow returns the number of rows that are present in a data frame. Here, it is returning the number of rows present in "coords" 
...
```{r}
#Our plot is created based on the dataframe of coords and heights by means of X and Y. The colour scale is dependent upon heights, as higher measurements are more red, while shorter men are documented as a more blue shade. 
ggplot() + 
...
```{r}
#Mutate function adds new variables while preserving esixting variables
#Subset function is a method of selecting variables
...
```{r}
#Here, we are establishing that pattern1 is "coords"
pattern1 <- coords
```
