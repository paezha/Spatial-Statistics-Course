...
```{r}
#Here, we are creating a data frame by means of X and Y in space, with repect to income by household.
...
```{r}
#The "ggplot" function defines household incomes in point space. Higer income households appear to be on the East regions of the area. 
...
```{r}
#creating zones with boundaries on both axes. "Zone_ID" only has 2 values because there are only two zones in the analysis. 
...
```{r}
#"ggplot" is used in this funciton to plot populations per census tract in Hamilton. This is visible as the "cut_number" function calls the population variable from the Hamilt census tract dataframe. "fill" is determined by population, resulting in higher population counts having a more red colour. 
...
```{r}
#Note how the "cut_number" calls population density rather than population like the figure above. This gives a more accurate representation of the distribution of population.
...
```{r}
#The "HAMILTON_CT" dataframe portions ages by category. For this choropleth map, we call all age categories over 65 by simply using the "+" sign. 
ggplot(Hamilton_CT) + 
...
```{r}
#The "Cartogram_cont" function construct a continuous area cartogram. Here, a cartogram is attempting to be constructed for the city of Hamilton by means of Census Tract.
...
```{r}
#"st_as_sf" function is converting "CT_pop_cartogram" to a "sf" object class so that we can successfully visualize the cartogram. 
