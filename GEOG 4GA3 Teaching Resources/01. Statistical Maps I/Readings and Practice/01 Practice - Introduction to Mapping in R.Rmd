---
title: "02 Practice - Introduction to Mapping in R"
...
Now that your workspace is clear, you can proceed to load a sample dataset. It is very important that R knows where to look for it. This means that the path to the file is explicit, or alternatively that you set the working directory to the directory where the file is. For instance, in my computer I would type:

```{r}
load("D:/Antonio/Courses/GEOG 4GA3 - Applied Spatial Analysis/Course Material EXPERIMENTAL/01. Statistical Maps/Snow.RData")
```

As an alternative, I could set the working directory to the directory where the file is, and then I could load it:
```{r}
rm(list = ls())
setwd("D:/Antonio/Courses/GEOG 4GA3 - Applied Spatial Analysis/Course Material EXPERIMENTAL/01. Statistical Maps")
load("Snow.RData")
```
