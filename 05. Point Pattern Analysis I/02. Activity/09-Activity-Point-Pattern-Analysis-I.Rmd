...
Next the geospatial files need to be read. For this example, an dataset that contains the city boundary of Toronto is used:
```{r}
data("Toronto")
data("Toronto_df")
```
...
As discussed in the preceding chapter, the package `spatstat` is a very rich collection of tools to do point pattern analysis. To convert the three sets of events (i.e., the fast food establishments, gas stands, and Paez Mart) into a ppp object use the following: 
```{r}
