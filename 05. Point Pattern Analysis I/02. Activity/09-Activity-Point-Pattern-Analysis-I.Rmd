...
```{r}
#Denisty map for Fast Food 
q_size <- 4000
ggplot() +
  geom_bin2d(data = data.frame(st_coordinates(Fast_Food)),
             aes(x = X, y = Y),
             binwidth = c(q_size, q_size)) +
  geom_sf(data = Toronto, color = "white", fill = NA) 

#Density map for Gas Stands
q_size <- 3000
ggplot() +
  geom_bin2d(data = data.frame(st_coordinates(Gas_Stands)),
             aes(x = X, y = Y),
             binwidth = c(q_size, q_size)) +
  geom_sf(data = Toronto, color = "white", fill = NA) 

#Denisty map for Paez Mart 
q_size <- 3500
ggplot() +
  geom_bin2d(data = data.frame(st_coordinates(Paez_Mart)),
             aes(x = X, y = Y),
             binwidth = c(q_size, q_size)) +
  geom_sf(data = Toronto, color = "white", fill = NA) 
```
...
```{r}
qcount_fastfood <- quadratcount(Fast_Food.ppp, nx = 80, ny = 80)
qcount_gasstands <- quadratcount(Gas_Stands.ppp, nx = 12.5, ny = 12.5)
qcount_paezmart <- quadratcount(Paez_Mart.ppp, nx = 12, ny = 12)
```
...
```{r}
table(qcount_fastfood)
table(qcount_gasstands)
table(qcount_paezmart)
```
...
```{r}
qtest_fastfood <- quadrat.test(Fast_Food.ppp, nx = 80, ny = 80)
qtest_fastfood
plot(qtest_fastfood)

qtest_gasstands <- quadrat.test(Gas_Stands.ppp, nx = 12.5, ny = 12.5)
qtest_gasstands
plot(qtest_gasstands)

qtest_paezmart <- quadrat.test(Paez_Mart.ppp, nx = 12, ny = 12)
qtest_paezmart
plot(qtest_paezmart)
```
