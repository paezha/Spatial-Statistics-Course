...
```{r}
xy_cbd <- c(-79.8708, 43.2584) #"c" is a function that combines it's arguments. Here, we are adding coordiates of Jackson Square as a vector to the dataframe. 
```
...
#we use 'spTransform' as a method for map projection by means of latitude, logitude and datum of WGS84. Remember, you can always learn more about the properties of a function by insterting a question mark before in front of the function (i.e ?coordinates)
...
```{r}
dist2cbd.sl <- distGeo(xy_ct, xy_cbd) #You can use 'distGeo' to calculate the shortest distance between two points on a centroid 
...
summary(model1) #'lm' is a formula we use for regression analysis in this chapter. Recall that 'dist.sl' was renamed from 'dist2cbd.sl', and is the distance from the CBD (jackson square)
```
...
```{r}
Hamilton_CT$model1.e <- model1$residuals #We are calling the residuals from 'model1' 
```
...
#Also recall that you can use 'poly2nb' to build a list of spatial weights based on regions with continous boundaries, like the polygons of Hamilton census tracts.
```
...
summary(model2) #Notce how the new 'model2' uses the inverse distance from the CBD rather than the original. This is because 'invdist.sl' is used in this model instead!
```
...
```{r}
Hamilton_CT <- mutate(Hamilton_CT, lnPOP_DEN = log(POP_DENSITY)) #We are mutating population density by taking its natural logarithm of both sides of the equation. This changes the coefficients back to a linear expression.
```
