...
1. How to estimate regression models in R.
2. About autocorrelation as a model diagnostic.
3. About variable transformations.
4. How to use autocorrelation analysis to improve regression models.
...
Finally, this function is used to create local Moran maps:
```{r}
...
As a graduate of SGES GEOG/ENVSC/EARTHSC 3MB3 Statistical Analysis, you are already familiar with regression analysis.
...
1) The linear form of the model is correct.
...
Lets add distance to the CBD as a covariate to your dataframe. We will use Jackson Square as the CBD of Hamilton. The function `geocode` from the `ggmap` package can be used to retrieved the coordinates of this location:
```{r}
xy_cbd <- geocode("Jackson Square, Hamilton, Ontario")
```

Further, the centroid coordinates of the census tracts can be retrieved from the `SpatialPolygonsDataFrame` by means of the function `spdedp::coordinates`:
```{r}
xy_ct <- coordinates(Hamilton_CT)
```

Given these coordinates, the function `distGeo` from the package `geosphere` can be used to calculate the great circle distance between the centroids of the census tracts and Hamilton's CBD:
```{r}
dist2cbd.sl <- distGeo(xy_ct, xy_cbd)
```

We can now append this variable to the dataframe for analysis:
```{r}
Hamilton_CT@data$dist.sl <- dist2cbd.sl
```

The arguments of the model include an object of type "formula" and a dataframe. Other arguments include conditions for subsetting the data, using sampling weights, and so on.
...
```{r}
model1 <- lm(formula = POP_DENSIT ~ dist.sl, data = Hamilton_CT@data)
summary(model1)
```
...
$$
P_i = f(D_i) + \epsilon_i = 4404.91 - 0.17895D_i + \epsilon_i
$$

# Autocorrelation as a model diagnostic

Let quickly explore the fit of the model. The scatterplot is of the actual population density, whereas the blue line is the regression model:
```{r}
ggplot(data = Hamilton_CT@data, aes(x = dist.sl, 
                                    y = POP_DENSIT)) + 
  geom_point() +
...
```{r}
Hamilton_CT@data$model1.e <- model1$residuals
Hamilton_CT.t <- left_join(Hamilton_CT.t, 
                           data.frame(TRACT = Hamilton_CT$TRACT, model1$residuals))
Hamilton_CT.t <- rename(Hamilton_CT.t, model1.e = model1.residuals)
```

And now we can create a map of the residuals, with red indicating negative residuals and blue positive:
```{r}
map_model1.e <- ggplot(data = Hamilton_CT.t, aes(x= long, y = lat, group = group,
                                                 fill = model1.e < 0)) +
  geom_polygon(color = "white") +
  scale_fill_manual(values=c("blue", "red")) +
  
  coord_equal()
ggplotly(map_model1.e) %>% layout(showlegend = FALSE)
```

Does the distribution of residuals look random?

Fortunately, we already have the tools to help us with this question.

Lets create a set of spatial weights:
```{r}
Hamilton_CT.w <- nb2listw(poly2nb(Hamilton_CT))
```
...
```{r}
Hamilton_CT@data <- mutate(Hamilton_CT@data, invdist.sl = 1/dist.sl)
```

Use inverse distance as the covariate in the model:
```{r}
model2 <- lm(formula = POP_DENSIT ~ invdist.sl, data = Hamilton_CT@data)
summary(model2)
```

As the scatterplot below shows (the model is the blue line), we can capture a non-linear relationship, which does a better job of describing the high density of tracts close to the CBD, but unfortunately is a poor description of low density tracts elsewhere:
```{r}
ggplot(data = Hamilton_CT@data, aes(x = dist.sl, 
                                    y = POP_DENSIT)) + 
  geom_point() +
  stat_function(fun=function(x)2303.1 + 2244627/x, geom="line", color = "blue", size = 1) +
  geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
```

Add the residuals of this model to the dataframe for further examination:
```{r}
Hamilton_CT@data$model2.e <- model2$residuals
Hamilton_CT.t <- left_join(Hamilton_CT.t, 
                           data.frame(TRACT = Hamilton_CT$TRACT, model2$residuals))
Hamilton_CT.t <- rename(Hamilton_CT.t, model2.e = model2.residuals)
```
...
```{r}
Hamilton_CT@data <- mutate(Hamilton_CT@data, lnPOP_DEN = log(POP_DENSIT))
```

And estimate the model:
```{r}
model3 <- lm(formula = lnPOP_DEN ~ dist.sl, data = Hamilton_CT@data)
summary(model3)
```
...
While this latest model provides a somewhat better fit, there is still systematic under- and over-prediction, as seen in the map below:
```{r}
Hamilton_CT.t <- left_join(Hamilton_CT.t, 
                           data.frame(TRACT = Hamilton_CT$TRACT, model3$residuals))
Hamilton_CT.t <- rename(Hamilton_CT.t, model3.e = model3.residuals)

map_model3.e <- ggplot(data = Hamilton_CT.t, aes(x= long, y = lat, group = group,
                                                 fill = model3.e < 0)) +
  geom_polygon(color = "white") +
  scale_fill_manual(values=c("blue", "red")) +
  
  coord_equal()
ggplotly(map_model3.e) %>% layout(showlegend = FALSE)
```

Moran's I as well strongly suggests that the residuals are not independent:
```{r}
Hamilton_CT@data$model3.e <- model3$residuals
moran.test(Hamilton_CT$model3.e, Hamilton_CT.w)
```

# A note about spatial autocorrelation in regression analysis

Spatial autocorrelation was originally seen as a problem in regression analysis. It is not difficult to see why, after testing three models in this section.

I prefer to view spatial autocorrelation as an opportunity for discovery. For instance, the models above all seem to struggle capturing the large variations in population density between the central parts and the suburbs of Hamilton. Perhaps this could be due to a _regime change_, that is, the underlying process operates somewhat differently in different parts parts of the city. The last model, for instance, suggests that Burlington might have an effect.
...
```{r}
POP_DEN.lm <- localmoran(Hamilton_CT$POP_DENSIT, listw = Hamilton_CT.w)
```
...
Notice how the model now allows for different slopes and intercepts for observations in different parts of the city.
...
```{r}
Hamilton_CT.t <- left_join(Hamilton_CT.t, 
                           data.frame(TRACT = Hamilton_CT$TRACT, model4$residuals))
Hamilton_CT.t <- rename(Hamilton_CT.t, model4.e = model4.residuals)

map_model4.e <- ggplot(data = Hamilton_CT.t, aes(x= long, y = lat, group = group,
                                                 fill = model4.e < 0)) +
  geom_polygon(color = "white") +
  scale_fill_manual(values=c("blue", "red")) +
  
  coord_equal()
ggplotly(map_model4.e) %>% layout(showlegend = FALSE)
```
...
```{r}
Hamilton_CT@data$model4.e <- model4$residuals
moran.test(Hamilton_CT$model4.e, Hamilton_CT.w)
```

Based on the results, we fail to reject the null hypothesis, and can be confident that the residuals are now random.

The following figure illustrates the final model:
```{r}
fun.1 <- function(x)exp(7.943 + 0.9537 - (0.00003247  + 0.0000273) * x) #CBD
fun.2 <- function(x)exp(7.943 - 0.9539 - (0.00003247 + 0.0001005) * x) #Suburb
fun.3 <- function(x)exp(7.943 - 0.00003247 * x) #Other

ggplot(data = Hamilton_CT@data, aes(x = dist.sl, y = POP_DENSIT)) +
  geom_point() +
  ...
  scale_color_manual(values = c("red", "blue", "black"), 
                     labels = c("CBD", "Suburb", "Other")) +
  geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
```

This concludes Practice 13.