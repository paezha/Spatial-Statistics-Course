...
```{r}
ggplot(missing_df) +
  geom_sf(aes(fill = cut_number(missing_df$VAR1, 5)), color = "black", alpha = 1, size = .3) +
...
```{r}
ggplot(PointPattern1) +
  geom_point(data = PointPattern1, aes(x=x, y=y), colour = 'blue', size =3)
ggplot(PointPattern2) +
  geom_point(data = PointPattern2, aes(x=x, y=y), colour = 'red', size =3)
ggplot(PointPattern3) +
  geom_point(data = PointPattern3, aes(x=x, y=y), colour = 'purple', size =3)
```
