...
* An Excel file called `Trips by Mode 2011.xlsx` with the number of trips by mode of transportation by TAZ, and other useful information from the 2011 census for Hamilton CMA.
...
```{r}
ggplot(data = Hamilton_TAZ.t, aes(x = long, y = lat, group = group.x, 
                                fill = Auto_driver.prop)) +
  geom_polygon() +
...
```{r}
moran.test(Hamilton_TAZ$Auto_driver.prop, Hamilton_TAZ.w)
```

3. Estimate regression model using the variables `Pop_Density` and travel time in `minutes`. Discuss this model.
```{r}
model <- lm(formula = Auto_driver.prop ~ Pop_Density + minutes, data = Hamilton_TAZ@data)
summary(model)
```
...
```{r}
moran.test(model$residuals, Hamilton_TAZ.w)
```
...
Create a histogram:
```{r}
ggplot(data = Hamilton_TAZ@data, aes(x = Auto_driver.prop)) + geom_histogram()
```

Scatterplots:
```{r}
ggplot(data = Hamilton_TAZ@data, aes(x = Pop_Density, y = Auto_driver.prop)) +
  geom_point()
```
...
Model:
```{r}
model2 <- lm(formula = log(Auto_driver.prop) ~ Pop_Density + minutes + Median_income + Employment_rate, data = Hamilton_TAZ@data)
summary(model2)
```

Test for spatial autocorrelation:
```{r}
moran.test(model2$residuals, Hamilton_TAZ.w)
```
