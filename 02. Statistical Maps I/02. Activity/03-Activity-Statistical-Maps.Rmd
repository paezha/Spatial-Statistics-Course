...
To create a siple map we can use `ggplot2`, which previously we used to map points. Now, the geom for sf can be used for areas. To create such a map, we layer a geom object of type `sf` on a `ggplot2` object. For instance, to plot the DAs:
```{r}
#head(HamiltonDAs)
ggplot(HamiltonDAs) + 
