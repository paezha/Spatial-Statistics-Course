...
In last previous practice/session, you learned about _area data_ and practiced some visualization techniques for spatial data of this type, specifically choropleth maps and cartograms. You also thought about rules to decide whether a mapped variable displayed a spatially random distribution of values. 
...
Consider the simple systems shown in Figure 1. Which of zones $A_2$, $A_3$, and $A_4$ is more _proximate_ to $A_1$?

![Figure 1. Simple zoning system](Area Data II Figure 1.jpg)
...
The first step to create a spatial weights matrix is to find the neighbors (i.e., areas adjacent to) for each area. The function `poly2nb` is used for this. The input argument is a `SpatialPolygonDataFrame`. You can verify that the object `Hamilton_CT` is a `SpatialPolygonDataFrame`:
```{r}
class(Hamilton_CT)
```
...
```{r}
plot(Hamilton_CT, border = "gray")
plot(Hamilton_CT.nb, coordinates(Hamilton_CT), col = "red", add = TRUE)
```
...
```{r}
map <- ggplot() +
  geom_polygon(data = Hamilton_CT.t, 
               aes(x = long, y = lat, group = group, 
                   fill = cut_number(POP_DENSIT, 5), 
                   POP_DENSIT = round(POP_DENSIT),
                   TRACT = TRACT), 
               color = "black") +
  geom_polygon(data = subset(Hamilton_CT.t, TRACT == "5370142.02"), 
               aes(x = long, y = lat, group = group, 
                   POP_DENSIT = round(POP_DENSIT),
                   TRACT = TRACT), 
               color = "red",
               weight = 3, fill = NA) +
    geom_polygon(data = subset(Hamilton_CT.t, TRACT == "5370144.01"), 
               aes(x = long, y = lat, group = group, 
                   POP_DENSIT = round(POP_DENSIT),
                   TRACT = TRACT), 
               color = "green",
...
```{r}
POP_DENSIT.sma <- lag.listw(x = Hamilton_CT.w, Hamilton_CT$POP_DENSIT)
```
...
map.sma <- ggplot() +
  geom_polygon(data = Hamilton_CT.t, 
               aes(x = long, y = lat, group = group, 
                   fill = cut_number(POP_DENSIT.sma, 5), 
                   POP_DENSIT.sma = round(POP_DENSIT.sma),
                   TRACT = TRACT), 
               color = "black") +
  geom_polygon(data = subset(Hamilton_CT.t, TRACT == "5370142.02"), 
               aes(x = long, y = lat, group = group, 
...
First we need to obtain the coordinates of the centroids of the areas:
```{r}
CT_centroids <- coordinates(Hamilton_CT)
```

A nearest neighbors object `nb` is produced as follows (selecting a distance threshold between 0 and 5 km):
```{r}
Hamilton_CT.dnb <- dnearneigh(CT_centroids, d1 = 0, d2 = 5, longlat = TRUE)
```
...
```{r}
Hamilton_CT.knb <- knn2nb(knearneigh(CT_centroids, k = 3, longlat = TRUE))
```
