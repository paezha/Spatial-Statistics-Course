...
```{r}
#dist2cbd.sl <- st_distance(Hamilton_CT, xy_cbd)
```

The distance is given in meters. Convert to numeric and divide by $1,000$ to convert to kilometers, before adding this variable to `Hamilton_CT` for analysis (I also add 1 meter to the distances to ensure that there are no zeros in that variable):
```{r}
