...
Mext. we add our new variable distance to CBD to our dataframe `Hamilton_CT` for analysis:
```{r}
...
# Recall that 'plot_ly()' is a function used to create interactive plots
  plot_ly(Hamilton_CT) %>% 
  # Recal that `add_sf()` is similar to `geom_sf()` and it draws a simple features object on a `plotly` plot. This example adds colours to represent positive (blue) and negative residuals (red).
...
We can visaully examine the spatial distribution of the residuals by means of the following map:
```{r message = FALSE, warning = FALSE}
...
ggplot(data = Hamilton_CT, aes(x = dist.sl, y = POP_DENSITY)) +
  ...
  # `stat_fuction()` draws custom functions on a `ggplot2` plot.
