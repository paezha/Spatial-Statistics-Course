...
```{r}
tor_nn <- nndist(Fast_Food.ppp)
ggplot() + geom_histogram(data = data.frame(dist=tor_nn), aes(dist), binwidth = 200) + 
  ggtitle("Nearest Neighbour Distance of Fast Food Restaurants in Toronto")
```

```{r}
plot(split(Fast_Food.ppp))

chicken_nn1 <- nndist(split(Fast_Food.ppp)$"Chicken")
hamburger_nn1 <- nndist(split(Fast_Food.ppp)$"Hamburger")
pizza_nn1 <- nndist(split(Fast_Food.ppp)$"Pizza")
sub_nn1 <- nndist(split(Fast_Food.ppp)$"Sub")

ggplot() + geom_histogram(data = data.frame(dist = chicken_nn1), aes(dist), binwidth = 40) + 
  ggtitle("Nearest Neighbour Distance of Fast Food Chicken Restaurants in Toronto")
ggplot() + geom_histogram(data = data.frame(dist = hamburger_nn1), aes(dist), binwidth = 40) + 
  ggtitle("Nearest Neighbour Distance of Fast Food Hamburger Restaurants in Torotno")
ggplot() + geom_histogram(data = data.frame(dist = pizza_nn1), aes(dist), binwidth = 40) + 
  ggtitle("Nearest Neighbour Distance of Fast Food Pizza Restaurants in Toronto")
ggplot() + geom_histogram(data = data.frame(dist = sub_nn1), aes(dist), binwidth = 40) + 
  ggtitle("Nearest Neighbour Distance of Fast Food Sub Restaurants in Toronto")
```
...
```{r}
plot(split(Fast_Food.ppp)$"Chicken" %mark% (chicken_nn1), markscale = 1, main = "Stienen diagram for Chicken")
plot(split(Fast_Food.ppp)$"Hamburger" %mark% (hamburger_nn1), markscale = 1, main = "Stienen diagram for Hamburger")
plot(split(Fast_Food.ppp)$"Pizza" %mark% (pizza_nn1), markscale = 1, main = "Stienen diagram for Pizza")
plot(split(Fast_Food.ppp)$"Sub" %mark% (sub_nn1), markscale = 1, main = "Stienen diagram for Sub")
```
...
```{r}
g_chicken <- Gest(split(Fast_Food.ppp)$"Chicken", correction = "none")
plot(g_chicken)
g_hamburger <- Gest(split(Fast_Food.ppp)$"Hamburger", correction = "none")
plot(g_hamburger)
g_pizza <- Gest(split(Fast_Food.ppp)$"Pizza", correction = "none")
plot(g_pizza)
g_sub <- Gest(split(Fast_Food.ppp)$"Sub", correction = "none")
plot(g_sub)
```
