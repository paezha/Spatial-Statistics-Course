...
This map is provides a decent example of a vicual representation of some contents in the dataframe. Here, information is displayed using different colours and symbols for pumps and deaths from the London Cholera Example. Though this map provides useful insight, it is not of the greatest quality. Lets further investigate packages that illustrate maps interactively. 
...
An alternative and quicker way to run the same bit of code is through using Pipe operators that make writing code a lot faster!:
```{r}
