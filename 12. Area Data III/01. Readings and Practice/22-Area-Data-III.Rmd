...
```{r}
#Poly2nb funciton builds a neighbours list based on regions with contiguous boundaries, where the argument is "Hamilton_CT.sp", an object containiing multi-polygon objects. 

#nb2listw supplements a neighbours list with spatial weights (for "Hamilton_CT.sp). These two functions create a spatial weights matrix for census tracts.
...
```{r}
#lag.listw function is establishing that there is a dependency between population density and census tracts in Hamilton. This can be further explained as one variable is affecting the result of another variable (how populaito density changes by each census tract). This is the spatial moving average. 
...
```{r}
#Taking a random sample which will be simulated further below. 
POP_DENSITY_s1 <- sample(Hamilton_CT$POP_DENSITY)
```

Calculate the spatial moving average for this randomized variable:
```{r}
#Calculating the spatial moving average for the sample variable from above. 
...
```{r}
#Note that we are creating 8 simulations of population density Spatial Moving Average of the Randomized variable using "lag.listw". Each simulation has a new name. 
...
```{r}
#"Hamilton_CT2 is created as a new choropleth map. Here, pipe operators (%>%) denote that we are selecting the empirical spatial moving average in addition to the 8 simulations of population densities of Hamilton census tracts. 
...
Hamilton_CT2 <- Hamilton_CT %>% select(POP_DENSITY.sma, 
                                       ...
                                       POP_DENSITY_s7.sma,
                                       POP_DENSITY_s8.sma,
                                       geometry) %>% #this pipe operator extacts density variables and geometry
...
```{r}
#decipher the formula. You are able to see that we are calculating the sum of the products by its spatial moving average, divided by variance
